...
class ProxyNetworkProvider {
    ...
    async estimateTransactionCost(tx) {
        const copiedTx = core_1.Transaction.newFromPlainObject(tx.toPlainObject());
        if (!copiedTx.nonce) {
        ...
        const transaction = core_1.prepareTransactionForBroadcasting(copiedTx);
        const response = await this.doPostGeneric("transaction/cost", transaction);
        const returnMessage = response.returnMessage;
        if (returnMessage) {
            throw new core_1.ErrEstimateTransactionCost(returnMessage);
        }
