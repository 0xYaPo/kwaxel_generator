...
describe('formatAmount', () => {
    ...
    test('handles zero values correctly', () => {
        expect((0, formatAmount_1.formatAmount)({ input: '0' })).toBe('0'); // 0 EGLD
        expect((0, formatAmount_1.formatAmount)({ input: '0', digits: 2 })).toBe('0'); // 0 EGLD
    });
    test('formats positive integer amounts without decimal places', () => {
        expect((0, formatAmount_1.formatAmount)({ input: '1000000000000000000' })).toBe('1'); // 1 EGLD
        expect((0, formatAmount_1.formatAmount)({ input: '2000000000000000000' })).toBe('2'); // 2 EGLD
    });
    ...
    test('handles custom decimals for different token types', () => {
        expect((0, formatAmount_1.formatAmount)({ input: '1000000000000000000', decimals: 8 })).toBe('10000000000'); // 10000000000 tokens (8 decimals)
        expect((0, formatAmount_1.formatAmount)({ input: '1000000000000000000', decimals: 4 })).toBe('100000000000000'); // 100000000000000 tokens (4 decimals)
        expect((0, formatAmount_1.formatAmount)({ input: '56817349973594872345', decimals: 18, digits: 4 }) // 56.817349973594872345 EGLD
        ).toBe('56.817349973594872345');
    });
    test('handles custom digits parameter', () => {
        expect((0, formatAmount_1.formatAmount)({ input: '1000000000000000000', digits: 2 })).toBe('1'); // 1 EGLD
        expect((0, formatAmount_1.formatAmount)({ input: '1000000000000000000', digits: 4 })).toBe('1'); // 1 EGLD
    });
    test('adds thousands separators (commas) when specified', () => {
        expect((0, formatAmount_1.formatAmount)({ input: '1000000000000000000000', addCommas: true }) // 1000 EGLD
        ).toBe('1,000');
